...
def train_data(X,y,encoder):
    ...
    os.makedirs('src/api/models', exist_ok=True)

    joblib.dump(model, 'src/api/models/streamer_model.pkl')
    joblib.dump(encoder, 'src/api/models/cat_encoder.pkl')
    joblib.dump(X.columns.tolist(), 'src/api/models/feature_names.pkl')
